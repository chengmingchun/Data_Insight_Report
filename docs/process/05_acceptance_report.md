...
| FR-07 | 通用 AI 洞察 + fallback | Insight | DeepSeek 默认预设、兼容服务适配、聚合输入、确定性降级 | 通过 |
...
| FR-11 | PDF 分析报告 | ReportLab | 3 页样例、视觉渲染检查、集成测试 | 通过 |

## 加分项

- 已实现：二维透视、IQR、异常 CSV 下载、数字幻觉校验、Dockerfile、交互 Plotly、完整分层测试代码。
- 已实现：PDF 分析报告，包含业务概览、洞察、质量、图表、Top-N、异常和处理规则。
